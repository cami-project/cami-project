...
def send_notification(user_id, type, status, message, description):
    timestamp = time.time()
    n = Notification(user_id=user_id, type=type, status=status, timestamp=timestamp, message=message, description=description)
    n.full_clean()
    n.save()