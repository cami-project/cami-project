...
static const system_clock::time_point deltaTimeBegin = std::chrono::high_resolution_clock::now();

template<>
string GetDate<Delta>(system_clock::time_point t) {
    return std::to_string(std::chrono::duration<double>(t - deltaTimeBegin).count());
}
